...
create index if not exists work_log_project_day_idx
  on work_log_entries(project_id, worked_on desc);
create index if not exists work_log_author_idx
  on work_log_entries(author_id, worked_on desc);
...
create table if not exists work_log_items (
  id uuid primary key default gen_random_uuid(),
  entry_id uuid not null references work_log_entries(id) on delete cascade,
  item_id uuid references resource_items(id) on delete cascade,
  url text,
  label text,
  constraint work_log_item_target check (num_nonnulls(item_id, url) = 1)
);

create index if not exists work_log_items_entry_idx on work_log_items(entry_id);
...
drop policy if exists "work_log_select" on work_log_entries;
create policy "work_log_select" on work_log_entries
...
drop policy if exists "work_log_insert" on work_log_entries;
create policy "work_log_insert" on work_log_entries
  for insert to authenticated with check (
    author_id = auth.uid()
    and (public.in_project(project_id) or public.is_project_admin(project_id))
  );

drop policy if exists "work_log_update" on work_log_entries;
create policy "work_log_update" on work_log_entries
  for update to authenticated
  using (author_id = auth.uid()) with check (author_id = auth.uid());

drop policy if exists "work_log_delete" on work_log_entries;
create policy "work_log_delete" on work_log_entries
  for delete to authenticated using (
    author_id = auth.uid() or public.is_project_admin(project_id)
  );

-- Attachments follow their entry.
drop policy if exists "work_log_items_select" on work_log_items;
create policy "work_log_items_select" on work_log_items
  for select to authenticated using (
    exists (
      select 1 from work_log_entries e
      where e.id = entry_id
        and (e.author_id = auth.uid() or public.is_project_admin(e.project_id))
    )
  );

drop policy if exists "work_log_items_write" on work_log_items;
create policy "work_log_items_write" on work_log_items
...
begin
  ...
  insert into notifications (type, title, message, subject_user_id, target_user_id, target_role)
  values (
    'work_logged',
...
drop trigger if exists work_log_notify on work_log_entries;
...
do $$
begin
  alter publication supabase_realtime add table work_log_entries;
